...
function EconomicCalendar() {
  ...
  // Strategy: use getTimezoneOffset() for local, and compute ET offset via
  // a known reference point parsed in NY timezone.
  // getTimezoneOffset() → minutes WEST of UTC (UTC-4 = +240, UTC+1 = -60)
  // ET (America/New_York): UTC-5 in EST, UTC-4 in EDT (daylight saving)
  // We calculate the actual current ET offset by using the browser's Intl
  // to get the UTC offset of NY right now, avoiding string parsing entirely.
  const etToLocalDiffMin = useMemo(() => {
    const now = new Date();
    // Local UTC offset in minutes (positive = behind UTC, e.g. UTC-4 → 240)
    const localOffsetMin = now.getTimezoneOffset();

    // Get ET offset: create a timestamp, format it in NY to get hour, compare with UTC hour
    // Use getUTCHours to get UTC reference, then format in NY to see the NY hour.
    // NY civil hour = UTC_hour - ET_offset_hours
    // ET_offset_hours = UTC_hour - NY_civil_hour  (in hours behind UTC, so positive for Americas)
    const utcH = now.getUTCHours();
    const utcM = now.getUTCMinutes();
    const utcTotalMin = utcH * 60 + utcM;

    // Get NY hours/minutes via Intl — only use it for NY, not for local (use getTimezoneOffset for local)
    let etOffsetMin;
    try {
      ...
      let nyH = parseInt(nyParts.find(p => p.type === "hour")?.value ?? "0");
      const nyM = parseInt(nyParts.find(p => p.type === "minute")?.value ?? "0");
      if (nyH === 24) nyH = 0; // some browsers return 24 for midnight
      const nyTotalMin = nyH * 60 + nyM;
      // ET offset = UTC - NY (minutes behind UTC)
      let rawDiff = utcTotalMin - nyTotalMin;
      if (rawDiff > 720)  rawDiff -= 1440;
      if (rawDiff < -720) rawDiff += 1440;
      etOffsetMin = rawDiff; // e.g. 240 for EDT (UTC-4)
    } catch {
      etOffsetMin = 240; // fallback: EDT (UTC-4), covers most of trading year
    }

    // diff = minutes to ADD to ET civil time to get local civil time
    // ET_civil = UTC - etOffsetMin/60h  →  local_civil = UTC - localOffsetMin/60h
    // local_civil - ET_civil = etOffsetMin - localOffsetMin
    return etOffsetMin - localOffsetMin;
  }, []);
